...
**KF_MODE 运行模式**（代码支持三种模式，生产部署使用 `full`）：
- `KF_MODE=chat`：加载用户流量路由（`routes_chat.py`）
- `KF_MODE=admin`：加载管理后台路由（`routes_admin.py`）
- `KF_MODE=full`（生产默认）：同时加载两组路由，单 Deployment 统一承载
...
kf-api 使用 `Dockerfile` 构建的镜像，`KF_MODE=full` 统一承载 chat + admin 全部路由。单 Deployment 简化架构，减少资源开销和维护成本。
